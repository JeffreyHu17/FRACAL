...
class ConvFCFracalBBoxHead(ConvFCBBoxHead):
    def __init__(self,
                 num_shared_convs=0,
                 num_shared_fcs=0,
                 num_cls_convs=0,
        ...
        if dataset == 'v3det':
            self.wy = self.get_fracal_weights('./stat_files/freq_v3det_train.csv',self.num_classes,variant)
            self.fractal_weights = pd.read_csv('./stat_files/v3det_train_fractal_dim.csv')['fractal_dimension'].values.tolist()
        elif dataset == 'openimages':
            self.wy = self.get_fracal_weights('./stat_files/freq_open_images.csv',self.num_classes,variant)
            self.fractal_weights = pd.read_csv('./stat_files/fractal_dims_open_images.csv')['fractal_dimension'].values.tolist()
        elif dataset == 'lvis':
            self.wy = self.get_fracal_weights('./stat_files/freq_lvis_v1_train.csv',self.num_classes,variant)
            self.fractal_weights = pd.read_csv('./stat_files/lvis_v1_train_fractal_dim.csv')['fractal_dimension'].values.tolist()
        elif dataset == 'coco':
            self.wy = self.get_fracal_weights('./stat_files/freq_coco_train.csv',self.num_classes,variant)
            self.fractal_weights = pd.read_csv('./stat_files/coco_train_fractal_dim.csv')['fractal_dimension'].values.tolist()
        if self.loss_cls.use_sigmoid is False:
            # self.fractal_weights = self.fractal_weights + [1.0]*(self.num_classes-len(self.fractal_weights)) +[1.0]
            self.fractal_weights = self.fractal_weights +[1.0]
    ...
    def get_fracal_weights(self,lvis_file,num_categories,variant):
...
class Shared2FCFracalBBoxHead(ConvFCFracalBBoxHead):
    ...
    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(Shared2FCFracalBBoxHead, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
